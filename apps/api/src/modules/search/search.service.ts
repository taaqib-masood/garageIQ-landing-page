...
Query: "${query}"`;
  ...
  private mockResolveQuery(query: string) {
  ...
  async searchGarages(dto: SearchQueryDto) {
    const startTime = Date.now();
    const { query, location, filters, page, limit } = dto;

    // 1. Resolve intent from query
    ...
    const locationTextCondition = intent.location_text && locationStrategy === 'text_match'
      ? Prisma.sql`AND (gl.area ILIKE ${'%' + intent.location_text + '%'} OR gl.emirate ILIKE ${'%' + intent.location_text + '%'} OR gl.city ILIKE ${'%' + intent.location_text + '%'})`
      : Prisma.empty;
    ...
    const scoredResults = (candidates as any[]).map(c => {
      ...
      // Weight by price intent
      let priceWeight = price;
      if (intent.price_band === 'budget') priceWeight = price * 1.3;
      if (intent.price_band === 'premium') priceWeight = (1 - price) * 1.3;
